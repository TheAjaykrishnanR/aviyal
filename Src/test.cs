...
class Injector : IDisposable
{
    ...
    /// </summary>
    private Injector(string moduleName, int pid)
    {
        this.moduleName = moduleName;

        modulePath = new FileInfo(moduleName).FullName;
        rp = new(pid);
    }
    ...
    public static Injector? New(string moduleName, int pid)
    {
        ...
        {
            WriteLine($"Export func: {MODULE_EXPORTED_INIT}() not present in {moduleName}");
            return null;
        }
        ...
        {
            WriteLine($"Export func: {MODULE_EXPORTED_SET_ARG}() not present in {moduleName}");
            return null;
        }
        ...
        {
            WriteLine($"Export func: {MODULE_EXPORTED_CLEAR_ARGS}() not present in {moduleName}");
            return null;
        }
        ...
        {
            WriteLine($"Export func: {MODULE_EXPORTED_MAIN}() not present in {moduleName}");
            return null;
        }
    ...
    public unsafe bool Inject()
    {
        ...
        {
            WriteLine("argsPtr = null");
            return false;
        }

        var result = rp.Call(fnPtr, argsPtr);

        WriteLine($"Inject(), fnPtr: {fnPtr}, argsPtr: {argsPtr}");
        WriteLine(
    ...
    public bool Unload()
    {
        ...
        WriteLine($"Unload(), fnPtr: {fnPtr}, argsPtr: {moduleBase}");
        WriteLine(
    ...
    public static void Main(string[] args)
    {
        ...
        nint hWnd = Convert.ToInt32(args[1]);

        Injector? injector = New("swda.dll", pid);
        if (!injector?.Inject() ?? false)
        {
            WriteLine("Injector failed");
            return;
        }

        // call injected dll's exported functions
        injector?.CallInit();
        injector?.CallSetArg(hWnd);
        injector?.CallSetArg(Convert.ToInt32(args[2]));
        injector?.CallMain();
