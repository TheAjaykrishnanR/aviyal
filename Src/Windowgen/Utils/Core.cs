...
public partial class Utils
{
    ...
    public static bool IsContextMenu(nint hWnd)
    {
        ...
        if (className == "SysShadow")
            return true;
        if (className == "TrayiconMessageWindow")
            return true;
        if (className == "tray_icon_app")
            return true;
        return false;
    }
    ...
    public static bool IsWindowVisible(nint hWnd)
    {
        ...
        if (styleList.Contains("WS_VISIBLE"))
            return true;
        return false;
    }
    ...
    public static List<GUIProcess> EnumWindowProcesses()
    {
        ...
        EnumWindowProc enumWindowProc = (nint hWnd, nint lParam) =>
        {
            ...
            if (
                (
                    guiProcess = guiProcesses
                        .Where(_p => _p.name == process.ProcessName)
                        .FirstOrDefault()
                ) == null
            )
            {
    ...
    public static string? GetExePathFromHWND(nint hWnd)
    {
        ...
        if (Environment.IsPrivilegedProcess)
        {
            ...
            Process? process = allWindows
                .Where(guiProcess => guiProcess.process.Id == processId)
                .FirstOrDefault()
                ?.process;
        ...
        //
        string? exePathDeviceName = driveDevicePaths
            .Where(path => exePath.Contains(path))
            .FirstOrDefault();
    ...
    public static int HideWindowInAltTab(nint hWnd)
    {
        ...
        return User32.SetWindowLong(
            ...
            (int)(
                (exStyles | (uint)WINDOWSTYLE.WS_EX_TOOLWINDOW) & ~(uint)WINDOWSTYLE.WS_EX_APPWINDOW
            )
        );
    }
    ...
    public static void StickWindowToBottom(nint hWnd)
    {
        ...
        User32.SetWindowLong(
            hWnd,
            (int)GETWINDOWLONG.GWL_EXSTYLE,
            (int)(exStyles | (uint)WINDOWSTYLE.WS_EX_NOACTIVATE)
        );
    }
