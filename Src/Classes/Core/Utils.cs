...
public class Utils
{
    ...
    public static void MoveWindowToCursor(nint hWnd, int offsetX = 0, int offsetY = 0)
    {
        ...
        User32.SetWindowPos(hWnd, nint.Zero, cursorPos.X + offsetX, cursorPos.Y + offsetY, 0, 0,
            SETWINDOWPOS.SWP_NOSIZE);
    }
    ...
    public static List<GUIProcess?> EnumWindowProcesses()
    {
        ...
        EnumWindowProc enumWindowProc = (hWnd, lParam) =>
        {
            ...
            {
                guiProcess = new GUIProcess { name = process.ProcessName };
    ...
    public static string? GetExePathFromHWND(nint hWnd)
    {
        ...
        if (Environment.IsPrivilegedProcess)
            try
            {
                ...
                var process = allWindows.FirstOrDefault(guiProcess => guiProcess?.process.Id == processId)?.process;
            ...
            catch (Exception ex)
            {
                ...
                // closing process
                Logger.Log("System.Diagnostics.Process gave up :(", ex);
            }
        ...
        SYSTEM_PROCESS_ID_INFORMATION info = new()
        {
            ...
            ImageName = new UNICODE_STRING { Length = 0, MaximumLength = 256, Buffer = Marshal.AllocHGlobal(512) }
        };
        var result = Ntdll.NtQuerySystemInformation(SYSTEM_INFORMATION_CLASS.SystemProcessIdInformation, ref info,
            (uint)Marshal.SizeOf<SYSTEM_PROCESS_ID_INFORMATION>(), out var returnLength);
        ...
        //
        var exePathDeviceName = driveDevicePaths.FirstOrDefault(path => exePath.Contains(path));
    ...
    public static bool IsWindowInTaskBar(nint hWnd)
    {
        ...
        var exStyle = (WINDOWSTYLEEX)User32.GetWindowLong(hWnd, GETWINDOWLONG.GWL_EXSTYLE);
    ...
    public static IPAddress GetLANIP()
    {
        return NetworkInterface.GetAllNetworkInterfaces()
            ...
            .Select(iface => iface.GetIPProperties().UnicastAddresses
                .Where(addr =>
                    addr.Address.AddressFamily == AddressFamily.InterNetwork && addr.PrefixOrigin == PrefixOrigin.Dhcp)
            )
    ...
    public static NetworkInterface GetPrimaryNetworkInterface()
    {
        ...
        return interfaces.First(iface =>
            iface.GetIPProperties().UnicastAddresses.Select(ucast => ucast.Address).Contains(addr));
    }
    ...
    public static void StickWindowToBottom(nint hWnd)
    {
        ...
        var exStyles = User32.GetWindowLong(hWnd, GETWINDOWLONG.GWL_EXSTYLE);
        User32.SetWindowLong(hWnd, (int)GETWINDOWLONG.GWL_EXSTYLE,
            (int)(exStyles | (uint)WINDOWSTYLEEX.WS_EX_NOACTIVATE));
    }
    ...
    public static bool IsProcessElevated(int pid)
    {
        ...
        Advapi32.GetTokenInformation(tokenHandle, TOKEN_INFORMATION_CLASS.TokenElevation, ref info, sizeof(uint),
