...
public partial class Utils
{
	...
	public static List<GUIProcess?> EnumWindowProcesses()
	{
		...
		EnumWindowProc enumWindowProc = (nint hWnd, nint lParam) =>
		{
			...
			if (
				(guiProcess = guiProcesses.FirstOrDefault(_p => _p.name == process.ProcessName))
				== null
			)
			{
	...
	public static string? GetExePathFromHWND(nint hWnd)
	{
		...
		int result = Ntdll.NtQuerySystemInformation(
			SYSTEM_INFORMATION_CLASS.SystemProcessIdInformation,
			ref info,
			(uint)Marshal.SizeOf<SYSTEM_PROCESS_ID_INFORMATION>(),
	...
	public static bool IsWindowInTaskBar(nint hWnd)
	{
		...
		WINDOWSTYLEEX exStyle = (WINDOWSTYLEEX)
			User32.GetWindowLong(hWnd, GETWINDOWLONG.GWL_EXSTYLE);
		if (exStyle.HasFlag(WINDOWSTYLEEX.WS_EX_TOOLWINDOW))
			return false;
	...
	public static IPAddress GetLANIP()
	{
		return NetworkInterface
			...
			.Select(iface =>
				iface
					.GetIPProperties()
					.UnicastAddresses.Where(addr =>
						addr.Address.AddressFamily == AddressFamily.InterNetwork
						&& addr.PrefixOrigin == PrefixOrigin.Dhcp
					)
			)
	...
	public static int HideWindowInAltTab(nint hWnd)
	{
		...
		return User32.SetWindowLong(
			...
			(int)(
				(exStyles | (uint)WINDOWSTYLEEX.WS_EX_TOOLWINDOW)
	...
	/// </summary>
	public static (int, int) ScaleImage(
		int imgWidth,
		int imgHeight,
		int targetWidth,
		int targetHeight
	...
	{
		string touchDimension =
			(targetWidth / imgWidth) > (targetHeight / imgHeight) ? "width" : "height";
		double scale =
			touchDimension == "width"
				? (double)targetWidth / imgWidth
				: (double)targetHeight / imgHeight;
	...
	public static string GetProcessUserName(string processName)
	{
		...
		Advapi32.OpenProcessToken(handle, TOKEN_QUERY, out nint tokenHandle);
		TOKEN_USER info = new();
		HandleWin32(Advapi32.GetTokenInformation(
			tokenHandle,
		...
		HandleWin32(Advapi32.LookupAccountSid(
			null,
			info.User.Sid,
			name,
			out uint _,
	...
	{
		Console.WriteLine($"{funcName}(): {returnValue}, LastWin32Error: {Marshal.GetLastWin32Error()}");
	}
}
