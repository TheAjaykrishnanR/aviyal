...
public class Dwindle : ILayout
{
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            switch (fillDirection)
            {
                case FillDirection.HORIZONTAL:
                    if (i - 1 >= 0) fillRects[i - 1] = TopHalf(fillRects[i - 1]);
                    fillRect.Left += (fillRect.Right - fillRect.Left) / 2;
                    break;
                case FillDirection.VERTICAL:
                    if (i - 1 >= 0) fillRects[i - 1] = LeftHalf(fillRects[i - 1]);
    ...
    private RECT LeftHalf(RECT rect)
    {
        rect.Right -= (rect.Right - rect.Left) / 2;
        return rect;
    }

    private RECT TopHalf(RECT rect)
    {
        rect.Bottom -= (rect.Bottom - rect.Top) / 2;
        return rect;
    }

    private EDGE[] GetEdges(RECT rect, int screenWidth, int screenHeight)
    {
        List<EDGE> edges = new();
