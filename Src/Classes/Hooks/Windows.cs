...
public class WindowEventsListener : IDisposable
{
    ...
    private readonly int CHILDID_SELF = 0;

    private readonly Lock eventLock = new();

    private readonly int OBJID_WINDOW = 0;
    private uint dt;
    private nint hhook;
    ...
    [DllImport("user32.dll", SetLastError = true)]
    private static extern int UnhookWinEvent(nint hhook);
            ...
            lock (eventLock)
            {
                ...
                switch (msg)
                {
                    ...
                    case WINEVENT.EVENT_OBJECT_LOCATIONCHANGE:
                        ...
                        if (state == SHOWWINDOW.SW_SHOWNORMAL) WINDOW_RESTORED(new Window(hWnd));
                        break;
