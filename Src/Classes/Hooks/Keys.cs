...
public class KeyEventsListener : IDisposable
{
    ...
    private const int HOTKEY_COMBO_TIMEOUT = 2000;

    private readonly List<VK> captured = new();
    private readonly List<Keymap> keymaps = new();
    private uint dt;

    private nint hhook;
    private bool hotkeyPressed; // if hotkey keys combo are remaining pressed
    ...
    private uint lastKeyTime;
    private bool letKeyPass = true;
    private bool running = true;

    public Thread thread;
    private VK? trailingKey; // the trailing key of a hotkey action -> H in Ctrl+Shift+H
    ...
    private static extern nint SetWindowsHookExA(int idHook, KeyboardProc lpfn, nint hmod, uint dwThreadId);
    ...
    private int KeyboardCallback(int code, nint wparam, nint lparam)
    {
        ...
        switch ((WINDOWMESSAGE)wparam)
        {
            case WINDOWMESSAGE.WM_KEYDOWN or WINDOWMESSAGE.WM_SYSKEYDOWN /* ALT */:
                if (!captured.Contains(key) && key != 0)
                    captured.Add(key);
                if (Aviyal.DEBUG) Logger.Log(captured, suffix: $"dt: {dt}");
                foreach (var keymap in keymaps)
                    if (Utils.ListContentEqual(captured, keymap.keys))
                    {
                        trailingKey = key;
                        letKeyPass = false;
                        captured.Remove(key);
    ...
    private void Loop()
    {
        ...
        hhook = SetWindowsHookExA(WH_KEYBOARD_LL, KeyboardCallback, Process.GetCurrentProcess().MainModule!.BaseAddress,
            0);
    ...
    private delegate int KeyboardProc(int code, nint wparam, nint lparam);
}
...
public class Keymap
{
    ...
    {
        if (((Keymap)obj).id == id) return true;
        return false;
    }
